...
function fv(pv: number, pmt: number, rate: number, years: number) {
  ...
  return pv * Math.pow(1 + rate, years) + pmt * ((Math.pow(1 + rate, years) - 1) / rate);
}
...
function YesNo({ label, value, onChange, tooltip }: {
        ...
        {([true, false] as const).map((b) => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-xs font-medium border transition-all ${
              value === b
                ? b ? "bg-green-600 text-white border-green-600" : "bg-red-500 text-white border-red-500"
...
function ResultRow({ label, value, color = "text-gray-900", bold = false }: {
...
export default function WealthSnapshotCalc() {
  ...
  const projectionBars = currentAge > 0 ? [
            ...
            {appetiteOpts.map((opt) => (
              <button
                ...
                className={`w-full text-left p-3 rounded-xl border-2 transition-all ${
                  form.riskAppetite === opt.value ? "border-primary bg-primary/5" : "border-gray-200 hover:border-gray-300"
                }`}
              >
                <div className="flex items-start gap-3">
                  <div className={`mt-0.5 h-4 w-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center ${
                    form.riskAppetite === opt.value ? "border-primary" : "border-gray-300"
                  }`}>
            ...
            {styleOpts.map((opt) => (
              <button
                ...
                className={`text-left p-3 rounded-xl border-2 transition-all ${
                  form.investmentStyle === opt.value ? "border-secondary bg-secondary/5" : "border-gray-200 hover:border-gray-300"
                }`}
              >
  ...
  const renderEstate = () => (
    <div className="space-y-2">
      <p className="text-xs text-gray-500 mb-3">Indicate which estate planning documents you currently have in place.</p>
        ...
        <Card>
          <CardHeader className="pb-2 pt-4 px-4">
            <CardTitle className="text-sm font-semibold flex items-center gap-2 text-primary">
              {(() => { const t = TABS.find(t => t.id === activeTab)!; const Icon = t.icon; return <><Icon className="h-4 w-4" />{t.label}</>; })()}
            </CardTitle>
          </CardHeader>
          ...
          <CardContent className="pt-4 px-4 pb-4">
            ...
            <div className="mt-3 space-y-0">
              ...
              <div className={`flex justify-between items-center pt-2 mt-1 border-t border-gray-200`}>
                ...
                <span className={`font-bold text-lg ${netWorth >= 0 ? "text-primary" : "text-red-600"}`}>{fmt(netWorth)}</span>
              </div>
            </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    {[
                      ...
                      <div key={label} className={`flex items-center gap-1.5 text-xs p-1.5 rounded ${val === true ? "text-green-700" : val === false ? "text-red-600" : "text-gray-400"}`}>
